...
import { Building, User, CheckCircle, XCircle, Clock, AlertTriangle } from 'lucide-react';
...
const AdminPanel = () => {
    ...
    const handleVerify = async (registrationId, action, rejectionReason = '') => {
        ...
        try {
            const response = await mainAPI.post('/blockchain-registration/company/verify', {
                registrationId,
                action,
                rejectionReason
            });
    ...
    if (loading) {
        return <LoadingSpinner size="lg" text="Đang tải danh sách yêu cầu đăng ký..." fullScreen />;
    }
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-600 mt-2">Quản lý yêu cầu đăng ký blockchain</p>
                    </div>
                    ...
                    registrations.map((registration) => (
                        <Card key={registration.id} className="hover:shadow-md transition-shadow">
                                    ...
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">
                                                {registration.user?.email} • {registration.role === 'company' ? 'Doanh nghiệp' : 'Sinh viên'}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {/* Company Details */}
                                    {registration.role === 'company' && registration.description && (
                                        <div>
                                            <h4 className="font-medium text-gray-900 mb-2">Mô tả công ty:</h4>
                                    ...
                                    {/* Student Skills */}
                                    {registration.role === 'student' && registration.skills && registration.skills.length > 0 && (
                                        <div>
                                    ...
                                    {/* Wallet Address */}
                                    <div>
                                        <h4 className="font-medium text-gray-900 mb-2">Địa chỉ ví:</h4>
