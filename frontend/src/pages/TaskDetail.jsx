...
const TaskDetail = () => {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      toast.success("Ứng tuyển thành công!", { id: "apply-success" });
      setShowApplyModal(false);
      setCoverLetter("");
      fetchTaskDetail(); // Refresh data
    } catch (error) {
      const message =
        error.response?.data?.error || "Có lỗi xảy ra khi ứng tuyển";
      toast.error(message, { id: "apply-error" });
    } finally {
      setApplying(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      {showWalletModal && (
        <Modal open={showWalletModal} onClose={() => setShowWalletModal(false)}>
          ...
          <CardContent>
            <p className="mb-4">
              Bạn cần liên kết ví MetaMask để ứng tuyển nhiệm vụ này.
            </p>
            <Button
