...
export const registerStudent = async (req, res) => {
    try {
        ...
        let blockchainResult = { success: false, error: 'Blockchain service not available' };
        let txHash = null;

        try {
            blockchainResult = await blockchainClient.registerStudent(name, skills, address);
...
export const registerCompany = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: 'Yêu cầu đăng ký blockchain đã được gửi và đang chờ admin duyệt',
            registration: {
                id: registration.id,
                status: registration.status
            }
...
export const verifyCompany = async (req, res) => {
    try {
        ...
        if (action === 'approve') {
            ...
            const blockchainResult = await blockchainClient.registerCompany(
                registration.name,
            ...
            if (!blockchainResult.success) {
                return res.status(500).json({
                    error: 'Lỗi đăng ký trên blockchain: ' + blockchainResult.error
                });
            }
            ...
            res.json({
                success: true,
                message: 'Đã duyệt đăng ký blockchain thành công',
                registration: {
                    id: registration.id,
                    status: registration.status,
                    txHash: registration.blockchain_tx_hash
