...
const queryValidation = [
    ...
    query('sort')
        ...
        .withMessage('Sort field must be created_at, deadline, reward_tokens, or title'),
    query('order')
        .optional()
        .isIn(['ASC', 'DESC'])
...
router.post('/', authenticate, authorize('company'), createTaskValidation, createTask);
router.put('/:id', authenticate, authorize('company'), updateTaskValidation, updateTask);
router.delete('/:id', authenticate, authorize('company'), taskIdValidation, deleteTask);
router.post('/:id/apply', authenticate, authorize('student'), applyValidation, applyToTask);
router.patch('/:id/applications/:applicationId/review', authenticate, authorize('company'), reviewValidation, reviewApplication);
router.patch('/:id/submit', authenticate, authorize('student'), submitWorkValidation, submitWork);
router.patch('/:id/confirm', authenticate, authorize('company'), confirmCompletionValidation, confirmCompletion);
